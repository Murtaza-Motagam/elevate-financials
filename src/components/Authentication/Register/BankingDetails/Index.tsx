          ...
          {loading ? (
            ...
            <div>
              <div className='w-full flex-col md:flex md:flex-row space-y-5 md:space-y-0 justify-center mb-10 md:mb-5 items-start md:justify-between'>
                ...
                <div className='text-xl md:text-2xl font-semibold border-b pb-2 border-gray-800 dark:border-gray-300'>
                  Account Overview
                </div>
                ...
                <div className='hidden md:block copyright'>
                  <div className='flex items-center gap-x-2'>
                    ...
                    <p className='text-xs'>
                      <span className='font-bold'>Copyright</span> {compNm} 2020, <br />
                      All rights reserved.
                    </p>
                  </div>
                </div>
              ...
              <div className='name flex flex-col items-center space-y-3'>
                {user?.documentDetails?.profileImg ? (
                  ...
                  <TextToImage
                    nameText={`${user?.personalDetails?.firstName} ${user?.personalDetails?.lastName}`}
                    className='rounded-full text-lg w-14 h-14 shadow-md shadow-gray-500'
                  />
                )}
              ...
              <div className='grid grid-cols-1 md:grid-cols-3 gap-4 my-3'>
                <Details
                  ...
                  text={user?.personalDetails?.email}
                />
                <Details
                  title='Username'
                  icon={<User size={15} className='text-primary' />}
                  text={user?.authentication?.username}
                />
                <Details
                  title='Account Number'
                  icon={<CircleUser size={15} className='text-primary' />}
                  text={user?.accountDetails?.accountNumber}
                />
                <Details
                  title='CRN Number'
                  icon={<CircleUser size={15} className='text-primary' />}
                  text={user?.accountDetails?.crnNumber}
                />
                <Details
                  title='IFSC Code'
                  icon={<CircleUser size={15} className='text-primary' />}
                  text={user?.accountDetails?.ifscCode}
                />
                <Details
                  title='Account Type'
                  icon={<HandCoins size={15} className='text-primary' />}
                  text={user?.accountDetails?.accountType}
                />
                <Details
                  title='Balance'
                  icon={<IndianRupee size={15} className='text-primary' />}
                  text={`${user?.accountDetails?.balance}/-`}
                />
                <Details
                  title='Account Status'
                  icon={<CircleCheck size={15} className='text-primary' />}
                  innerClass='text-green-500 !font-extrabold  uppercase'
              ...
              <div className='w-full flex flex-col space-y-2 md:space-y-0 md:flex-row items-center justify-end space-x-2'>
                <DefaultButton
                  onClick={handleCopy}
                  icon={copy ? <ClipboardCheck /> : <Clipboard />}
                  title={copy ? 'Copied' : 'Copy Details'}
                  className='text-xs w-full md:w-fit'
                />
                <DefaultButton
                  variant='outline'
                  ...
                  icon={<Home />}
                  title='Go to Home'
                  className='w-full md:w-fit text-xs'
                />
              </div>
              <div className='footer-container mt-6 p-4 border-t border-gray-200 dark:border-gray-700'>
                ...
                <p className='text-xs text-gray-500 dark:text-gray-500 text-center mt-2'>
                  This information is provided for your reference. Please review your account
                  details carefully. If you notice any discrepancies, contact our support team
                  immediately.
                </p>
                <p className='text-xs text-gray-500 dark:text-gray-500 text-center mt-1'>
                  Your account information is encrypted and secure. For added safety, avoid sharing
