...
export const documentDetails = yup.object().shape({
    ...
    panNo: yup.string().matches(alphanumeric, "Pancard number must be alphanumeric").required("Pancard number is required"),
...
export const bankingDetails = yup.object().shape({
    ...
    password: yup.string().matches(passwordValidate, 'Password should must validate').required("Account type is required"),
});